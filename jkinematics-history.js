...
(function(window) {
    ...
    // Global variables
    var console = window.console || {}, document = window.document, History = window.History;
    ...
    var browserBackButtonSupport = isDefined(window.history.pushState);
        ...
        if (isDefined(this.currentStep) && isDefined(this.currentStep.phases)
            ...
            do {
                ...
                if (!ret && !this.currentPhase.skipped) {
                    ...
                    if ((changementPhase || isUndefined(previousPhase))
                            && (browserBackButtonSupport || isDefined(History))) {
                        var state = new State();
    ...
    if (browserBackButtonSupport) {
        JKinematics.Adapter.attach(window, 'popstate', function(event) {
            handleStateChange(event.state);
        }, false);
    } else if (isDefined(History) && History.enabled) {
